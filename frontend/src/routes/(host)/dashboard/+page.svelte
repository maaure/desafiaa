...
<div class="px-8 py-8 max-w-5xl">
  ...
  <div class="flex items-center justify-between mb-8">
    ...
    <a
      ...
      class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-violet-600 text-white text-sm font-semibold
  ...
  {:else}
    <div class="grid gap-4" style="grid-template-columns: repeat(auto-fill, minmax(320px, 1fr))">
      {#each quizzes as quiz (quiz.id)}
        <div
          class="group bg-white rounded-xl border border-slate-200 hover:border-violet-200 hover:shadow-md
          ...
          <div class="px-5 py-3 border-t border-slate-100 flex items-center gap-2">
            ...
            {#if quiz.isPublished}
              <button
                ...
                class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg
                  text-xs font-semibold text-white bg-violet-600 hover:bg-violet-700 active:bg-violet-800 transition-colors shadow-sm"
              >
